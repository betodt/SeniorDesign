...
An activity diagram is a flowchart of a user\textsc{\char13}s actions to accomplish a goal. Our first figure, figure \ref{activityOne}, depicts the actions necessary to create a project. 
...
\item[A.] Project with the same name already exists
\begin{enumerate}
\item[1.] System shows failure message
\item[2.] User must rename his project
\end{enumerate}
}
...
Figure \ref{ad} shows a highlevel overview of the technologies that make up our application.
...
\par The backend of our application is provided by Firebase, a cloud base Backend-as-a-service, which handles our realtime database. The front-end is built using Ember.js, a JavaScript framework based on the Model-View-ViewModel architecture. The two are coupled together through the built-in data adapter, appropriately named EmberFire, which translates the JSON objects provided by Firebase into models compatible with Ember.js. Ember.js uses the ViewModel layer to manage the users interactions with the View and provide the View with the appropriate information from the Model.
...
\item The system aims to experience minimal latency from user input to interface response. Our application needs to be as immersive as possible. Any technical glitch will detract from the user\textsc{\char13}s experience.
\item The system\textsc{\char13}s design follows conventional web application layouts. Proper placement of features was the difference between leaving users confused and onboarding new users so quickly that they begin planning activities in a matter of minutes.
\end{enumerate}
\end{enumerate}
\end{enumerate}